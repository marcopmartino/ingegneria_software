...
class ShoeLastVariety:
    ...
    def get_description(self) -> str:
        description_abbozzo = (f"{self.__product_type.value} da {self.__gender.value}, {self.__shoe_last_type.value}, "
                               f"plastica tipo {str(self.__plastic_type.value)}")
        if self.__product_type == ProductType.ABBOZZO:
            return description_abbozzo

        else:
            description_abbozzo_sgrossato = (f"{description_abbozzo}, taglia {self.__size}" +
                                             ("" if self.__processing == Processing.NESSUNA else
                                              f", lavorazione {self.__processing.name.lower().replace("_", " ")}"))

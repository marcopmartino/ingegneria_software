...
class EditWorkerProfileWindow(QMainWindow):
    ...
    def __init__(self, prevWindow, user_id: str, parent=None):
        ...
        self.workerRadio.setChecked(True) if data['role'] == 'worker' else self.adminRadio.setChecked(True)

        self.radioBox.addWidget(self.workerRadio, alignment=Qt.AlignLeft)
        self.radioBox.addWidget(self.adminRadio, alignment=Qt.AlignRight)
    ...
    def btnState(self, button):
        if button.text() == "Operario":
            ...
            if not button.isChecked():
                self.workerRadio.setChecked(False)
